...
export PROJECT_VERSION=0.31.1
export JAVA_HOME=/lib/jvm/java-1.8.0
export ENFORCE_VERSION=

set -- "$@" assemble uploadArchives -DdisableTests 
...
if [ -n "" ]; then
  echo "Creating tag 0.31.1 to match enforced version"
  git tag -m 0.31.1 -a 0.31.1 || true
fi
